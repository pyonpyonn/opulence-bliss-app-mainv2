...
export default function SessionCountdown({ bookingId, status, startedAt, durationMinutes }: {
  ...
  }, [status, startedAt, durationMinutes, bookingId]);
  ...
  return <section style={{ padding: 18, margin: "16px 0", borderRadius: 14, background: seconds <= 1800 ? "#fff3d6" : "#f3edff", color: "#382251" }}>
    ...
    <p role="status" style={{ margin: "4px 0" }}>{seconds === 0 ? "The booked cleaning duration has ended. The visit will check out automatically after 10 minutes unless it is finished sooner." : seconds <= 1800 ? "30 minutes or less remain in this session." : "Time remaining is based on the customer's chosen cleaning duration from check-in."}</p>
  </section>;
}
