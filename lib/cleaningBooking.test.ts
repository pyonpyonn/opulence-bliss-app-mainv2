...
test("cleaning has exactly 17 valid half-hour choices", () => {
  assert.equal(CLEANING_DURATIONS.length, 17);
  for (const minutes of [120,150,180,210,240,270,300,330,360,390,420,450,480,510,540,570,600]) assert.equal(validCleaningDuration(minutes), true);
  for (const minutes of [0,60,119,170,225,601,630,NaN,Infinity]) assert.equal(validCleaningDuration(minutes), false);
});
...
test("property-size guidance rounds up and caps at ten hours", () => {
  ...
  assert.equal(recommendedCleaningMinutes(500), 600);
});
test("cleaning hourly rates and checkout totals stay proportional", () => {
  ...
  assert.equal(bookingPricePence(pkg, 600), 25000);
